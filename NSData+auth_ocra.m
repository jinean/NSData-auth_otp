...
#import "NSData+auth_ocra.h"
enum {
    OCRANumberOfDigitsTooLargeError = 100
};

@implementation NSData (auth_ocra)
...
{
    return [self dynamicOcraPasscode:NSDATA_AUTH_OCRA_SUITE_DEFAULT counter:nil question:question password:nil sessionInformation:nil timestamp:nil error:error];
}

- (NSString *)dynamicOcraPasscode:(NSString*)ocraSuite question:(NSString*)question error:(NSError**) error
{
    return [self dynamicOcraPasscode:ocraSuite counter:nil question:question password:nil sessionInformation:nil timestamp:nil error:error];
}
                            ...
                            error:(NSError**)error;
{
    ...
    if(counterLength > 0 ) {
        bArray = [NSData hexToBytes:counter];
    ...
    if(questionLength > 0 ) {
        bArray = [NSData hexToBytes:question];
    ...
    // Input is HEX encoded
    if(passwordLength > 0) {
        bArray = [NSData hexToBytes:password];
    ...
    if(sessionInformationLength > 0 ) {
        bArray = [NSData hexToBytes:sessionInformation];
    ...
    if(timeStampLength > 0) {
        bArray = [NSData hexToBytes: timeStamp];
